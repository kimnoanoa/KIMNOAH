...
이 과정을 통해 단순히 기능 구현이 아닌, 서비스를 완성도 있게 운영하는 개발자의 관점을 배우게 되었습니다.
  ...
  특히 예약 로직과 결제 시스템을 직접 구현하는 과정에서 트랜잭션 관리, 예외 처리, 서비스 계층 분리(Controller → Service → Repository(DB)) 등 안정적인 서비스 운영을 위한 설계 패턴을 익혔습니다.

  또한 Thymeleaf를 이용해 프론트엔드와 백엔드를 통합하며 MVC 구조의 흐름을 명확히 이해할 수 있었고,
  반응형 웹을 적용하면서 실제 사용자 환경에 맞는 UI·UX를 고려하는 능력을 기르게 되었습니다.

  첫 Spring Boot 협업 프로젝트로서, 백엔드와 프론트엔드 간의 역할을 명확히 구분하고,
  API 명세를 기반으로 데이터 흐름을 조율하는 과정을 통해 의사소통의 중요성과 팀 단위 개발의 효율성을 실감했습니다.
  서로의 개발 속도와 구조를 맞추기 위해 지속적으로 피드백을 주고받으며
  실제 서비스 환경에서의 협업 프로세스를 경험할 수 있었습니다.

  외부 API(Kakao, Naver, Google) 연동 과정에서 OAuth 인증 구조와 REST 통신의 흐름을 직접 경험하며,
  API 요청·응답 구조와 보안 토큰 관리의 개념을 구체적으로 습득할 수 있었습니다.

  마지막으로 문제 해결 과정에서 단순히 기능 구현을 넘어서,
  서비스 품질을 유지하면서 확장 가능한 구조로 개선해 나가는 사고방식과 협업을 통한 시너지의 가치를 함께 배우게 되었습니다.
    `
  },
...
서비스를 안정적으로 운영하기 위한 개발자의 책임감과 세부 설정의 중요성을 배우게 되었습니다.
  ...
  burger: {
    title: "Burger&Chicken HTML 웹사이트",
    tech: "HTML, CSS, JS",
